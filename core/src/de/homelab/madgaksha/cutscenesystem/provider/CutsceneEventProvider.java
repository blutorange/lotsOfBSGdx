...
public interface CutsceneEventProvider {
	...
	/** Called when the cutscene layer is added to the stack and this provider must finish all necessary setup. */
	public void initialize();

	/** Called when the cutscene layer is removed from the the stack and this provider must finish all necessary cleanup. */
	public void end();

	/**
	 * Called when the cutscene event has finished is not needed any longer.
	 * Must call any required cleanup on cutscene events. If this provider caches
	 * cutscene events, eg. because they are reused, cleanup must be done during {@link #end()}.
	 * @param currentCutsceneEvent The cutscene event that has just finished.
	 */
