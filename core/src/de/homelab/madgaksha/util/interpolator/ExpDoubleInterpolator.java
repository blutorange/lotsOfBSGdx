...
/**
 * It takes the exponential between x=[x1..x2] and maps its function values to to the range [a..b]. 
 *
 * {@see ILogInterpolatorOptions}
 * 
 * a... starting value
 * b... ending value  
 * x1.. exp map start     x1 <= 1
 * x2.. exp map end   0 < x2 <  1
 *
 * Good values for x1 and x2 are (0,2.71..5).
 *
 * The following function is used for interpolation:
 *   f(x) = (exp(x1+(x2-x1)*x)-exp(x1))/(exp(x2)-exp(x1))*(b-a)+a
//        = k1*exp(k2+k3*x)+k4
 * with
 *   k1 = (b-a)/(exp(x2)-exp(x1))
 *   k2 = x1
 *   k3 = (x2-x1)
 *   k4 = -exp(x1)/(exp(x2)-exp(x1))*(b-a)+a
 *   
 * @author madgaksha
 *
 */
