...
public class Score {
	...
	/**
	 * Writes the current score for the current level to the
	 * highscore file. Does nothing if the current score
	 * is lower than the highscore.
	 */
	...
	/**
	 * Writes the current score for the given level to the
	 * highscore file. Does nothing if the current score
	 * is lower than the highscore.
	 * @param level Level to which this score belongs.
	 */
	public void writeScore(InputStream scoreInputStream, OutputStream scoreOutputStream, ALevel level) {
		...
		else scoreMap.put(levelClass, score);
	...
	/**
	 * Reads the score file and returns is as a map with the level class as the key and the score as the value.
	 * @return The score for each level. Empty map when file not found or if it could not be read.
	 */
			...
			try {
				...
				do {
					...
					try {
						...
						Class<? extends ALevel> levelClass = (Class<? extends ALevel>)ClassReflection.forName(levelName);
						scoreMap.put(levelClass, score);
					}
