...
public class FadeInAimTrajectory extends BulletTrajectoryMaker {
	...
	protected IBehaving getBehaviour() {
		return aim != null ? new BulletBehaviour(aim, timeFadeIn, timeWaitAfterFadeIn, velocityAim, waveFrequency, waveAmplitude, soundOnFire, voicePlayer) : null;
	}
	...
	private final static class BulletBehaviour implements IBehaving {
		...
		public BulletBehaviour(Entity aim, float timeFadeIn, float timeWaitAfterFadeIn, float velocity, float waveFrequency, float waveAmplitude, ESound soundOnFire, VoicePlayer vp) {
			this.aim = aim;
		...
		public boolean behave(Entity bullet) {
			...
			case 0:
				...
				else ac.alpha = tc.totalTime / timeFadeIn;
				break;
			case 1:
			...
			case 2:
				if (aim != null) {
					if (voicePlayer != null) voicePlayer.play(soundOnFire);
