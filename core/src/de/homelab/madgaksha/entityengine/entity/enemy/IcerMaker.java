...
import de.homelab.madgaksha.entityengine.entity.trajectory.LinearAtTargetTrajectory;
...
public class IcerMaker extends NormalEnemyMaker {
	...
	private final static ESound soundList[] = new ESound[]{
			ESound.ICER_TOKUGI_0,
			ESound.ICER_TOKUGI_1,
		};
	private final static float intervalRatio[] = new float[] {
			0.45f,
			1.0f,
	};
	private final static ETexture signList[] = new ETexture[]{
		ETexture.ICER_TOKUGI_SIGN_0,
		ETexture.ICER_TOKUGI_SIGN_1,
	};
	private final static BulletShapeMaker[][] bulletListList = {
		new BulletShapeMaker[]{
				BulletShapeMaker.NOTE_RED,
				BulletShapeMaker.NOTE_BLUE,
				BulletShapeMaker.NOTE_YELLOW,
				BulletShapeMaker.NOTE_PINK,
		},
		new BulletShapeMaker[]{
				BulletShapeMaker.PLASMA_RED,
				BulletShapeMaker.PLASMA_BLUE,
				BulletShapeMaker.PLASMA_GREEN,
				BulletShapeMaker.PLASMA_YELLOW,
		}
	};
	...
	private static class MyBehaviour implements IBehaving {
		...
		public MyBehaviour(MapProperties props) {
			...
			if (bulletIntervalMin>=bulletIntervalMax) bulletIntervalMin = 0.5f*bulletIntervalMax;
